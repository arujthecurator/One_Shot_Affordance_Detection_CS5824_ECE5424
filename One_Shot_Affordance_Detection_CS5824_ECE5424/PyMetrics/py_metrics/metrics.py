...
class IoU(object):
    ...
    def cal_iou(self, pred: np.ndarray, gt: np.ndarray) -> np.ndarray:
        """
        ...
        #gt=np.where(gt==True,1,0)
        #print(pred)
        Iand1 = np.sum(gt * pred)

        #print(Iand1)
        Ior1 = np.sum(gt) + np.sum(pred) - Iand1
        IoU1 = Iand1 / Ior1
...
class CC(object):
    ...
    def step(self, pred: np.ndarray, gt: np.ndarray):
        ...
        cc = self.cal_cc(pred, gt)
        if cc>=-1 and cc<=1:
            self.CCs.append(cc)

    def cal_cc(self, pred: np.ndarray, gt: np.ndarray) -> np.ndarray:
        """

        """
        """
        mean_x=np.mean(np.mean(pred,axis=1),axis=0)
        mean_y=np.mean(np.mean(gt,axis=1),axis=0)
        top_sum=0
        x_pow=0
        y_pow=0
        for i in range(pred.shape[0]):
            for j in range(pred.shape[1]):
                top_sum+=(pred[i][j]-mean_x)*(gt[i][j]-mean_y)
                x_pow+=(pred[i][j]-mean_x)**2
                y_pow+=(pred[i][j]-mean_y)**2
        p=top_sum/np.sqrt(x_pow*y_pow)
        """
        pred= pred.flatten()
        gt=gt.flatten()
    ...
    def get_results(self) -> dict:
        ...
        :return: dict(mae=mae)
        """
        cc = np.mean(np.array(self.CCs, _TYPE))
        return dict(cc=cc)