...
class PLM(nn.Module):
    def __init__(self,input_channels=2048):
        ...
        self.pro=nn.Sequential(
          nn.Conv2d(self.input_channels,512,kernel_size=1,stride=1,padding=0),
	  nn.Conv2d(512,512,kernel_size=2,stride=1,padding=0),
          norm_layer(512),
          nn.ReLU(inplace=True))
        self.obj_pro=nn.Sequential(
          nn.Conv2d(self.input_channels,512,kernel_size=3,stride=1,padding=1),
	  nn.Conv2d(512,512,kernel_size=4,stride=1,padding=1),
          norm_layer(512),
          nn.ReLU(inplace=True))
        self.human_pro=nn.Sequential(
          nn.Conv2d(self.input_channels,512,kernel_size=3,stride=1,padding=1),
	  nn.Conv2d(512,512,kernel_size=4,stride=1,padding=1),
          norm_layer(512))
        self.conv=nn.Conv2d(512,1,kernel_size=3,stride=1,padding=1)
